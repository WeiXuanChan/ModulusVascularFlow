...
class PROPcombine(torch.nn.Module):
    def __init__(self,in_feature,out_feature,layer_sizeList,coord_nodes,mixinglength):
        super().__init__()
        self.start_end = torch.nn.ModuleList()
        self.start_end.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[out_feature]))#0
        self.start_end.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[out_feature]))#1
        
        self.layers = torch.nn.ModuleList()
        for n in range(len(coord_nodes)-1):
            self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
        ...
        mixinglength=4./mixinglength
        self.softplus=torch.nn.Softplus(beta=mixinglength, threshold=20/mixinglength)
        mixingomega=torch.tensor(4./mixinglength,dtype=torch.float)
        self.register_buffer("mixingomega", mixingomega, persistent=False)
        self.mixinglength=mixinglength
    def forward(self,x):#caseID
        clineclamp=self.coord_nodes[0,0]-self.softplus(self.coord_nodes[0,0]-x[...,0:1])
        y=self.start_end[0](torch.cat((clineclamp,x[...,1:]),dim=-1))
        for n in range(self.num-1):
            clineclamp=self.coord_nodes[0,n+1]-self.softplus(self.coord_nodes[0,n+1]-x[...,0:1])
            y=y+self.layers[n](torch.cat((clineclamp,x[...,1:]),dim=-1))*torch.tanh((x[...,0:1]-self.coord_nodes[0,n])*self.mixingomega)
        y=y+self.start_end[1](x)*torch.tanh((x[...,0:1]-self.coord_nodes[0,-1])*self.mixingomega)
        return y
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-12]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    channel_checkpoint_nd=tuple(map(lambda x: nd.ndim(quantity(x, "m")), np.linspace(-0.15,0.15,9)[1:-1]))
    channel = tubeWall(
        channel_center_nd,
    ...
    result_net=CustomModuleArch(
        input_keys=[Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")]+incrorderVar+[Key(x) for x in list(param_ranges_key.keys())],
        output_keys=[Key("predu"),Key("predv"),Key("predw"),Key("p")],
        module=PROPcombine(19,4,[256,512,512,512,512,512],channel_checkpoint_nd,nd.ndim(quantity(0.005, "m"))),
    )
