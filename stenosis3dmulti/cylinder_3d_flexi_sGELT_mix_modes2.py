...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-10]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    result_net = FullyConnectedFlexiLayerSizeArch(
        ...
        layer_sizeList=[total_modes,total_modes,total_modes,total_modes,total_modes],#20096256 trainables
    ...
    modeweights_net = HypernetFullyConnectedFlexiLayerSizeArch(
        ...
        layer_sizeList=[512,512,512,24],#814112 trainables
    ...
    # make solver
    slv = Solver_ReduceLROnPlateauLoss(cfg, domain,batch_per_epoch=1000*batchsizefactor*batchreductionfactor/8,ReduceLROnPlateau_DictConFig={'eps':1e-10})
    print("start solve")
    # start solver
    slv.solve()
