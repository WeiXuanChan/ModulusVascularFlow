...
def run(cfg: ModulusConfig) -> None:
    ...
    bdp_multiplier=2
    boundary_pts=volume_geo.sample_boundary(4000*int(bdp_multiplier))
    inlet_pts=tubeinlet.sample_boundary(500*int(bdp_multiplier))
    outlet_pts=tubeoutlet.sample_boundary(500*int(bdp_multiplier))
    total_pts=5000*int(bdp_multiplier)
    addstr=''
    if bdp_multiplier!=1:
        addstr='x'+str(bdp_multiplier)
    if not(os.path.isfile("/examples/stenosis3dmulti/ssboundary_pts"+addstr+".npy")):
        ...
        np.save("/examples/stenosis3dmulti/ssboundary_pts"+addstr+".npy",caseBoundarypt)
        return 0
    else:
        caseBoundarypt=np.load("/examples/stenosis3dmulti/ssboundary_pts"+addstr+".npy")
    ...
    nodes=nodes, invar=openfoam_invar_numpy, output_names=['warpx','warpy','warpz',"ref_vec_x","ref_vec_y","ref_vec_z","ref_arearatio","cline", "radius_y", "radius_z", "vec_x", "vec_y", "vec_z", "dissq","arearatio","ref_dissq","ref_vec_x","ref_vec_y","ref_vec_z"]
    )
