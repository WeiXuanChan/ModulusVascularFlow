...
def run(cfg: ModulusConfig) -> None:
    ...
    print("Copy file ","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/result_net.0.pth")
    shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-14]+"/result_net.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/result_net.0.pth")
    ...
    openfoam_invar_numpy["s1"]= np.repeat([5.],total_pts).reshape((-1,1))
    openfoam_invar_numpy["s3"]= np.repeat([4,],total_pts).reshape((-1,1))
    openfoam_invar_numpy["A"]= np.repeat([0.5],total_pts).reshape((-1,1))
    if True:
        ...
        openfoam_invar_numpy["s1"]= np.repeat([2.5],total_pts).reshape((-1,1))
        openfoam_invar_numpy["s3"]= np.repeat([2.5],total_pts).reshape((-1,1))
        openfoam_invar_numpy["A"]= np.repeat([0.55],total_pts).reshape((-1,1))
        openfoam_inferencer=PointwiseInferencer(
        nodes=nodes, invar=openfoam_invar_numpy, output_names=["u_scaled", "v_scaled", "w_scaled", "p_scaled",'warpx','warpy','warpz',"ref_vec_x","ref_vec_y","ref_vec_z","cline", "radius_y", "radius_z", "vec_x", "vec_y", "vec_z", "dissq"]
        )
        domain.add_inferencer(openfoam_inferencer, "training_data")
    # make solver
