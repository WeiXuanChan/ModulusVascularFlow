...
def run(cfg: ModulusConfig) -> None:
    ...
    PN_net = CustomModuleArch(
        [Key("caseID")],
        [Key(x+"_weights",size=total_modes) for x in GE_features],
        module=PointNet(caseBoundarypt,
    ...
    get_feature_net=[]
    for ge_feature in GE_features:
        get_feature_net.append(SumMultiplicationArch(
            input1_keys=[Key("modes",size=total_modes)],
            input2_keys=[Key(ge_feature+"_weights",size=total_modes)],
            output_keys=[Key(ge_feature)],
        ))
    ...
    print("get ge_feature nodes")
    for ge_featureN in range(len(GE_features)):
        nodes=nodes+[get_feature_net[ge_featureN].make_node(name='get_'+GE_features[ge_featureN]+'_net')]
    # make domain
    domain = Domain()
    x, y,z = Symbol("x"), Symbol("y"), Symbol("z")
    batchsizefactor=10
