...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-14]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    latentarray=np.load("/examples/stenosis3dmulti/sGEPN_mix_eval_latent.npy")
    getlatent_net = CaseIDtoFeatureArch(
        Key("caseID"),
        [Key("latent",size=16)],
        latentarray
        )
    ...
    nodes = (
        ...
        + [getlatent_net.make_node(name="getlatent_net")]
