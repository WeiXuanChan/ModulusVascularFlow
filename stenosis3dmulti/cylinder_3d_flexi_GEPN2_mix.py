...
class PointNet2(torch.nn.Module):
    def __init__(self,caseBoundarypt,pn_layer_sizeList1a,pn_layer_sizeList1b,pn_layer_sizeList2,total_modes,ge_features):
        super().__init__()
        caseNum=caseBoundarypt.shape[0]
        self.layers = torch.nn.ModuleList()
        self.layers.append(CaseIDArchCore(caseNum))#0
        self.layers.append(FixedFeatureArchCore(caseBoundarypt))#1
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = 3,
                                                                layer_sizeList = pn_layer_sizeList1a))#2
        self.layers.append(MaxFeaturePoolArchCore(pooldim=1,abs_op=True))#3
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = pn_layer_sizeList1a[-1]*2,
                                                                layer_sizeList = pn_layer_sizeList1b+[1]))#4
        self.layers.append(MaxPoolArchCore(pooldim=1,keepdim=False))#5
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = pn_layer_sizeList1a[-1],
                                                                layer_sizeList = pn_layer_sizeList2+[total_modes*ge_features],))#4
        
    def forward(self,x):#caseID
        caseID_unit=self.layers[0](x)#(training points,caseNum)
        caseBoundarypt=self.layers[1]()[...,:3]#(caseNum,bdpts,case_xyz)
        ptFeaturelatent1=self.layers[2](caseBoundarypt)#(caseNum,bdpts,case_features)
        ptFeaturelatent2=self.layers[3](ptFeaturelatent1)#(caseNum,case_features*2,case_features)
        ptlatent=self.layers[4](torch.transpose(ptFeaturelatent2,1,2)).squeeze(2)#(caseNum,case_features)
        latent0=self.layers[5](ptlatent)#(caseNum,case_features)
        latent=self.layers[6](latent0)#(caseNum,bdpts,case_features)
        TPlatent=torch.matmul(caseID_unit,latent)
        return TPlatent
import itertools
...
def run(cfg: ModulusConfig) -> None:
    ...
    total_pts=5000
    if not(os.path.isfile("/examples/stenosis3dmulti/boundary_pts.npy")):
        ...
        np.save("/examples/stenosis3dmulti/boundary_pts.npy",caseBoundarypt)
        return 0
    else:
        caseBoundarypt=np.load("/examples/stenosis3dmulti/boundary_pts.npy")
    total_modes=512
    ...
    PN_net = CustomModuleArch(
        [Key("caseID")],
        [Key("latent",size=16)],
        module=PointNet2(caseBoundarypt,
    ...
    flow_net = FullyConnectedArch(
        ...
        output_keys=[Key(x) for x in GE_features],
        layer_size=1680,
        nr_layers=4,
    )
    ...
    nodes=nodes, invar=openfoam_invar_numpy, output_names=['warpx','warpy','warpz',"ref_vec_x","ref_vec_y","ref_vec_z","ref_arearatio","cline", "radius_y", "radius_z", "vec_x", "vec_y", "vec_z", "dissq","arearatio"]
    )
