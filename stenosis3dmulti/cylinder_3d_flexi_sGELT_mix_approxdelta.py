...
class PointNet(torch.nn.Module):
    ...
    def forward(self,x):#caseID
        ...
        return TPlatent
class cossqrelu(torch.nn.Module):
...
class APPDcombine(torch.nn.Module):
    def __init__(self,in_feature,out_feature,layer_sizeList,coord_nodes,umax):
        super().__init__()
        self.incrOrder = torch.nn.ModuleList()
        self.incrOrder.append(incrorder3D())#Key("cline"),Key("radius_y"),Key("radius_z"),Key("dissq")
        self.layers = torch.nn.ModuleList()
        for n in range(len(coord_nodes)-1):
            self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature+8-5,
                                                                    layer_sizeList = layer_sizeList+[out_feature]))#n
        self.num=len(coord_nodes)
        coord_nodes=torch.tensor(np.array(coord_nodes).reshape((1,-1)),dtype=torch.float)
        self.register_buffer("coord_nodes", coord_nodes, persistent=False)
        umax=torch.tensor([[umax,0.,0.]],dtype=torch.float)
        self.register_buffer("umax", umax, persistent=False)
    def forward(self,x):#caseID
        xx=x[...,1:-5]#remove cline, vec_x,vec_y,vec_z,arearatio,hb
        vec_c=x[...,-5:-2]
        yuvw=torch.zeros_like(x[...,0:3])
        yp=torch.zeros_like(x[...,0:1])
        for n in range(self.num-1):
            clineclamp=torch.clamp(x[...,0:1], min=self.coord_nodes[0,n], max=self.coord_nodes[0,n+1])
            invclineclamp=self.coord_nodes[0,n+1]-clineclamp
            clineclamp=clineclamp-self.coord_nodes[0,n]
            newxx=torch.cat((clineclamp,xx),dim=-1)
            incrx=self.incrOrder[0](newxx[...,:4])
            ytemp=self.layers[n](torch.cat((newxx,incrx),dim=-1))
            yuvw=yuvw+ytemp[...,0:3]*clineclamp
            yp=yp+ytemp[...,3:4]*invclineclamp
        yuvw=yuvw*x[...,-1:]+self.umax*x[...,3:4]
        vec_rz=torch.cat((-vec_c[...,2:3],torch.zeros_like(vec_c[...,1:2]),vec_c[...,0:1]),dim=-1)/torch.sqrt(vec_c[...,0:1]**2.+vec_c[...,2:3]**2.)
        vec_ry=torch.cross(vec_rz,vec_c)
        yuvw=torch.matmul(torch.stack((vec_c,vec_ry,vec_rz),dim=x.dim()),yuvw.unsqueeze(-1)).squeeze(-1)/x[...,-2:-1]
        return torch.cat((yuvw,yp+ 0.25*self.umax[0,0]**2.*(1.-1./x[...,-2:-1]**2.)),dim=-1)
class APPDcombine0(torch.nn.Module):
    def __init__(self,in_feature,out_feature,layer_sizeList,coord_nodes,umax):
        super().__init__()
        self.incrOrder = torch.nn.ModuleList()
        self.incrOrder.append(incrorder3D())#Key("cline"),Key("radius_y"),Key("radius_z"),Key("dissq")
    ...
    def forward(self,x):#caseID
        ...
        yp=torch.zeros_like(x[...,0:1])
        clineclamp=x[...,0:1]
        invclineclamp=self.coord_nodes[0,1]-clineclamp
        clineclamp=clineclamp-self.coord_nodes[0,0]
        newxx=torch.cat((clineclamp,xx),dim=-1)
        incrx=self.incrOrder[0](newxx[...,:4])
        ytemp=self.layers[0](torch.cat((newxx,incrx),dim=-1))
        yuvw=yuvw+ytemp[...,0:3]*clineclamp
        yp=yp+ytemp[...,3:4]*invclineclamp
        yuvw=yuvw*x[...,-1:]+self.umax*x[...,3:4]
        vec_rz=torch.cat((-vec_c[...,2:3],torch.zeros_like(vec_c[...,1:2]),vec_c[...,0:1]),dim=-1)/torch.sqrt(vec_c[...,0:1]**2.+vec_c[...,2:3]**2.)
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-15]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    channel_checkpoint_nd=tuple(map(lambda x: nd.ndim(quantity(x, "m")), np.linspace(-0.15,0.15,2)))
    channel = tubeWall(
        channel_center_nd,
    ...
    flow_net = FullyConnectedArch(
        input_keys=[Key("x_case"), Key("y_case"), Key("z_case")]+[Key(x) for x in list(param_ranges_key.keys())],
    ...
    #    module=incrorder3D(),
    #)
    result_net=CustomModuleArch(
        input_keys=[Key("cline"),Key("radius_y"),Key("radius_z"),Key("dissq")]+[Key(x) for x in list(param_ranges_key.keys())]+[Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("arearatio"),Key("hb")],
        output_keys=[Key("u"),Key("v"),Key("w"),Key("p")],
        module=APPDcombine0(12,4,[2048,2048,2048,2048,2048,2048],np.linspace(-1.,1.,2),nd.ndim(inlet_u)),
    )
    ...
    x, y,z = Symbol("x"), Symbol("y"), Symbol("z")
    batchsizefactor=20
    batchreductionfactor=8##!!different from mix_full
    fixed_dataset=False
    quasirandom=True
    # inlet
    ...
    sameQ_outlet=IntegralBoundaryConstraint(
        ...
        batch_size=1,
        integral_batch_size=int(cfg.batch_size.outlet),#6400 = int(cfg.batch_size.outlet),
