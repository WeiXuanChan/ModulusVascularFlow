...
from modulusDL.models.arch import (
    ...
    MultiplicationArch,
    HypernetFullyConnectedFlexiLayerSizeArch
)
...
param_ranges_key["s1"]=np.array([2.25,2.75,3.25,3.75,4.25,4.75,5.25,5.75])
param_ranges_key["s3"]=np.array([2.25,2.75,3.25,3.75,4.25,4.75,5.25,5.75])
param_ranges_key["A"]=np.array([0.225,0.275,0.325,0.375,0.425,0.475,0.525,0.575])
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-6]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    result_net = FullyConnectedFlexiLayerSizeArch(
            input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")]+incrorderVar,
            output_keys=[Key("modes",size=total_modes)],
            layer_sizeList=[2048,2048,2048,2048,2048,2048],
        )
    modeweights_net = HypernetFullyConnectedFlexiLayerSizeArch(
        input_keys=[Key(x) for x in list(param_ranges_key.keys())]+[Key("caseID")],
        output_keys=[Key("modeweights_u",size=total_modes),Key("modeweights_v",size=total_modes),Key("modeweights_w",size=total_modes),Key("modeweights_p",size=total_modes)],
        layer_sizeList=[512,512,512,512,512,128],#32733 trainable weights
    ...
    get_u_net = SumMultiplicationArch(
        ...
        output_keys=[Key("predu")],
    )
    get_v_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("modeweights_v",size=total_modes)],
        output_keys=[ Key("predv")],
    )
    get_w_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("modeweights_w",size=total_modes)],
        output_keys=[ Key("predw")],
    )
    ...
    nodes = (
        ...
        +[get_w_net.make_node(name='get_w_net')]
    ...
    # make solver
    slv = Solver_ReduceLROnPlateauLoss(cfg, domain,batch_per_epoch=1000*batchsizefactor*batchreductionfactor/4,ReduceLROnPlateau_DictConFig={"min_lr":cfg.optimizer.lr*0.001})
    print("start solve")
    # start solver
    slv.solve()
