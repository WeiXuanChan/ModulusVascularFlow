...
class PointNet(torch.nn.Module):
    ...
    def forward(self,x):#caseID
        ...
        return TPlatent
class PIcombine(torch.nn.Module):
    def __init__(self,in_feature,number_modes,layer_sizeList):
        super().__init__()
        self.layers = torch.nn.ModuleList()
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[number_modes]))#0
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[number_modes]))#1
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[number_modes]))#2
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = in_feature,
                                                                layer_sizeList = layer_sizeList+[number_modes]))#3
        self.layers.append(FullyConnectedFlexiLayerSizeArchCore(in_features = number_modes,
                                                                layer_sizeList = [4]))#4
...
class velInit(torch.nn.Module):
    def __init__(self,u_max):
        super().__init__()
        p1=torch.tensor(3.96907794)
        self.register_buffer("p1", p1, persistent=False)
        p0=torch.tensor(11.858543046162612)
        self.register_buffer("p0", p0, persistent=False)
        u_max=torch.tensor(u_max)
        self.register_buffer("u_max", u_max, persistent=False)
    def forward(self,x):#dissq,arearatio,vecx,vecy,vecz,Vin,cline,A
        vel=self.u_max*(1+x[...,5:6])*x[...,0:1]/x[...,1:2]
        init_p=self.p0*torch.exp(x[...,7:8]*self.p1)
        p=init_p*(1.-x[...,6:7])*0.5 + 0.25*self.u_max**2.*(1.-1./x[...,1:2]**2.)
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-11]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    
    # physical quantities
    nu = quantity(0.0038, "kg/(m*s)")#dynamic viscosity
    ...
    result_net=CustomModuleArch(
        input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")]+incrorderVar+[Key(x) for x in list(param_ranges_key.keys())],
        output_keys=[Key("predu"),Key("predv"),Key("predw"),Key("dp")],
        module=PIcombine(22,1024,[512,1024,1024,1024,1024]),
    )
    initVel_module=CustomModuleArch(
        [Key("dissq"),Key("arearatio"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("Vin"),Key("cline"),Key("A")],
        [Key("u0"),Key("v0"),Key("w0"),Key("p0")],
        module=velInit(nd.ndim(inlet_u))
        )
    ...
    x, y,z = Symbol("x"), Symbol("y"), Symbol("z")
    batchsizefactor=20
    batchreductionfactor=10#!!!!different to mix_full
    fixed_dataset=False
    quasirandom=True
    # inlet
