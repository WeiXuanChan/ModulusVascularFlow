...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-19]+"/flow_network.0.pth","/examples/stenosis3dmulti/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    ...
    channel_checkpoint_nd=tuple(map(lambda x: nd.ndim(quantity(x, "m")), np.linspace(-0.15,0.15,9)))
    channel = tubeWall(
        channel_center_nd,
    ...
    result_net=CustomModuleArch(
        ...
        module=APPDcombine(12,4,[256,512,512,512,512,512],np.linspace(-1.,1.,9),nd.ndim(inlet_u)),
    )
    ...
    x, y,z = Symbol("x"), Symbol("y"), Symbol("z")
    batchsizefactor=20
    batchreductionfactor=16##!!different from mix_full
    fixed_dataset=False
    quasirandom=True
    # inlet
