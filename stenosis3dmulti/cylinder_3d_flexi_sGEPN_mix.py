...
class PointNet(torch.nn.Module):
    def __init__(self,bpts_sample,caseBoundarypt,pn_layer_sizeList1,pn_layer_sizeList2,total_modes,ge_features):
        ...
        self.layers.append(NominalSampledFixedFeatureArchCore(caseBoundarypt,bpts_sample))#1
    ...
    def forward(self,x):#caseID
        ...
        return TPlatent
    def randPointUpdate(self):
        caseBoundarypt=self.layers[1].allpts()[...,:3]#(caseNum,bdpts,case_xyz)
        ptlatent=self.layers[2](caseBoundarypt)#(caseNum,bdpts,case_features)
        latent0, max_indices=torch.max(ptlatent,1,keepdim=False)
        self.layers[1].updateSampleWeight(max_indices)
...
def run(cfg: ModulusConfig) -> None:
    ...
    '''
    if eval_latent:
        print("Total Number of case in eval",caseID_param_values.shape[0])
        for n in range(caseBoundarypt.shape[0]):
            runevalID=n
            if not(os.path.isfile("/examples/stenosis3dmulti/outputs/cylinder_3d_flexi_sGEPN_mix/inferencers/eval_latent"+str(runevalID)+".vtp")):
                break
        print("Running runevalID",runevalID)
        caseBoundarypt=caseBoundarypt[runevalID:runevalID+1]
    '''
    total_modes=1024
    ...
    if eval_latent:
        ...
        if runevalID==64:
            ...
            interior_pts=volume_geo.sample_interior(10)
            total_pts=20
        '''
        ...
        if False:#runevalID==64:
            openfoam_inferencer=PointwiseInferencer(
            nodes=nodes, invar=openfoam_invar_numpy, output_names=["caseID","latent"]+GE_features
            )
        else:
            openfoam_inferencer=PointwiseInferencer(
...
if __name__ == "__main__":
    ...
    if eval_latent:
        ...
        for n in range(1,65):
            run()
            if os.path.isfile("/examples/stenosis3dmulti/outputs/cylinder_3d_flexi_sGEPN_mix/inferencers/eval_latent64.vtp"):
                break
        '''
        if eval_latent:
            import vtk
            ...
            for n in range(1):
                ...
                reader.SetFileName("/examples/stenosis3dmulti/outputs/cylinder_3d_flexi_sGEPN_mix/inferencers/eval_latent_all.vtp")#"+str(n)+"
                reader.Update()
            ...
            np.save("/examples/stenosis3dmulti/outputs/cylinder_3d_flexi_sGEPN_mix/inferencers/eval_latent_all",latent_all)
