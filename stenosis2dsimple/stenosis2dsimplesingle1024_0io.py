...
from modulusDL.models.arch import CustomModuleArch, InheritedFullyConnectedFlexiLayerSizeArch, FullyConnectedFlexiLayerSizeArch, SingleInputInheritedFullyConnectedFlexiLayerSizeArch, CaseIDtoFeatureArch
...
class Stenosis(torch.nn.Module):
    ...
    def forward(self,x):
        x_ref=x[...,0:1]
        y_ref=x[...,1:2]
        y_case=y_ref*(1.-self.A/0.05*torch.exp(-1.*(x_ref/self.l)**2./(2.*self.sigma**2.)))
...
class GE(torch.nn.Module):
    ...
    def forward(self,x):
        x_case=x[...,0:1]
        y_case=x[...,1:2]
        radius=y_case/(1.-self.A/0.05*torch.exp(-1.*(x_case/self.l)**2./(2.*self.sigma**2.)))/self.r
...
param_ranges = {
    Symbol("A"): np.array([[0.015],
                           [0.022],
                           [0.028],
                           [0.035]]),
    Symbol("sigma"): np.array([[0.1],
                           [0.122],
                           [0.148],
                           [0.18]]),
}
...
def run(cfg: ModulusConfig) -> None:
    ...
    Stenosis_coordTransform=CustomModuleArch(
        ...
        module=Stenosis(nd.ndim(channel_radius),channel_length_nd[1]-channel_length_nd[0],A,sigma)
        )
    ge_net=CustomModuleArch(
        [Key("x_case"), Key("y_case")],
        [Key("cline"), Key("radius")],
        module=GE(nd.ndim(channel_radius),channel_length_nd[1]-channel_length_nd[0],A,sigma)
        )
    ...
    flow_net = FullyConnectedFlexiLayerSizeArch(
        input_keys=[Key("x_case"), Key("y_case")],
        output_keys=[Key("u"), Key("v"), Key("p")],
        layer_sizeList=[1024]*4,#3154947 trainable weights
