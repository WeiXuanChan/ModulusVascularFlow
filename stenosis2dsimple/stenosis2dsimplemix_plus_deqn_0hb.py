...
def run(cfg: ModulusConfig) -> None:
    ...
    for pthfile in pthfiles:
        if not(os.path.isfile("/examples/stenosis2dsimple/outputs/"+folder+str(os.path.basename(__file__))[:-3]+"/"+pthfile)):
    ...
    flow_net = FullyConnectedFlexiLayerSizeArch(
        input_keys=[Key("x_case"), Key("y_case"),Key("cline"), Key("radius"),Key("dissq"),Key("cline_sq"),Key("radius_sq"),Key("cline_radius"),Key("cline_dissq"),Key("radius_dissq"),Key("A"), Key("sigma")],
        output_keys=[Key("u"), Key("v"), Key("p")],
        layer_sizeList=[856]*4,#2214475 trainable weights
    ...
    openfoam_invar_numpy={"x":np.concatenate((interior_pts["x"],boundary_pts["x"]),axis=0),
                          ...
                          "A": np.repeat([0.025],total_pts).reshape((-1,1)),
                          "sigma": np.repeat([0.134],total_pts).reshape((-1,1)),
                          }
