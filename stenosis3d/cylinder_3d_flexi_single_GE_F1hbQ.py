...
from modulusDL.models.archCombi import (
    FBarch,
)
...
def run(cfg: ModulusConfig) -> None:
    ...
    result1_net = FullyConnectedArch(
    ...
    result2_net =FullyConnectedFlexiLayerSizeFBWindowArch(startList=[-1],
                                                        endList=[1.],
                                                        overlapRatio=0.2,
                                                        levels_num=2,
                                                        input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")],
                                                        ...
                                                        FB_keys=[Key("cline")],
                                                        layer_sizeList= [256,256,256,256]
                                                        )
    result8_net =FullyConnectedFlexiLayerSizeFBWindowArch(startList=[-1],
                                                        endList=[1.],
                                                        overlapRatio=0.2,
                                                        levels_num=8,
                                                        input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")],
                                                        output_keys=[Key("u_8"),Key("v_8"),Key("w_8"),Key("p_8")],
                                                        FB_keys=[Key("cline")],
                                                        layer_sizeList= [256,256,256,256]
                                                        )
    result16_net =FullyConnectedFlexiLayerSizeFBWindowArch(startList=[-1],
                                                        endList=[1.],
                                                        overlapRatio=0.2,
                                                        levels_num=16,
                                                        input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")],
                                                        output_keys=[Key("u_16"),Key("v_16"),Key("w_16"),Key("p_16")],
                                                        FB_keys=[Key("cline")],
                                                        layer_sizeList= [256,256,256,256]
                                                        )
    Upool_net=MeanPoolArch(
        input_keys=[Key("u_1"),Key("u_2"),Key("u_8"),Key("u_16")],
        output_keys=[Key("preu")],
        pooldim=-1,
        keepdim=True,
        keepview=False,
    )
    Vpool_net=MeanPoolArch(
        input_keys=[Key("v_1"),Key("v_2"),Key("v_8"),Key("v_16")],
        output_keys=[Key("prev")],
        pooldim=-1,
        keepdim=True,
        keepview=False,
    )
    Wpool_net=MeanPoolArch(
        input_keys=[Key("w_1"),Key("w_2"),Key("w_8"),Key("w_16")],
        output_keys=[Key("prew")],
        pooldim=-1,
        keepdim=True,
        keepview=False,
    )
    Ppool_net=MeanPoolArch(
        input_keys=[Key("p_1"),Key("p_2"),Key("p_8"),Key("p_16")],
        output_keys=[Key("p")],
        pooldim=-1,
        keepdim=True,
        keepview=False,
    ...
    sethb_net = MultiplicationArch(
        input1_keys=[Key("preu"),Key("prev"),Key("prew")],
        input2_keys=[Key("hb")],
    ...
    nodes = (
        ...
        + [result1_net.make_node(name="result1_net")]
        + [result2_net.make_node(name="result2_net")]
        + [result8_net.make_node(name="result8_net")]
        + [result16_net.make_node(name="result16_net")]
        + [Upool_net.make_node(name="Upool_net")]
        + [Vpool_net.make_node(name="Vpool_net")]
        + [Wpool_net.make_node(name="Wpool_net")]
        + [Ppool_net.make_node(name="Ppool_net")]
    ...
    inlet = PointwiseBoundaryConstraint(
        ...
        batch_size=int(cfg.batch_size.inlet/reduceSize),
        parameterization=param_ranges,
        batch_per_epoch= 1000*batchsizefactor*reduceSize,
    )
    ...
    no_slip_wall = PointwiseBoundaryConstraint(
        ...
        batch_size=int(cfg.batch_size.walls/reduceSize),
        parameterization=param_ranges,
        batch_per_epoch= 1000*batchsizefactor*reduceSize,
    )
    ...
    interior = PointwiseInteriorConstraint(
        ...
        batch_size=int(cfg.batch_size.interior/reduceSize),
    ...
    sameQ_outlet=IntegralBoundaryConstraint(
        ...
        batch_size=1,
        integral_batch_size=int(cfg.batch_size.outlet/reduceSize),
        batch_per_epoch= 100*batchsizefactor*reduceSize,
