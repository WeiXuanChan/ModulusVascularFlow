...
from modulusDL.models.arch import (
    ...
    AdditionArch,
    MultiplicationArch,
    FullyConnectedFlexiLayerSizeFBWindowArch
)
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3d/outputs/"+sys.argv[0][:-7]+"_backup/flow_network.0.pth","/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
        print("Copy file ","/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/result_net.0.pth")
        shutil.copy("/examples/stenosis3d/outputs/"+sys.argv[0][:-7]+"_backup/result_net.0.pth","/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/result_net.0.pth")
    # physical quantities
    ...
    refine1_net=FullyConnectedFlexiLayerSizeFBWindowArch(startList=[-0.5],
                                                        endList=[0.5],
                                                        overlapRatio=0.2,
                                                        levels_num=1,
                                                        input_keys=[Key("x_case"), Key("y_case"), Key("z_case"),Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq"),Key("arearatio")],
                                                        output_keys=[Key("predu_refine1"),Key("predv_refine1"),Key("predw_refine1"),Key("predp_refine1")],
                                                        FB_keys=[Key("cline")],
                                                        layer_sizeList= [256,256,256,256,256,256]
                                                        )
    refine1presure_net=FullyConnectedFlexiLayerSizeFBWindowArch(startList=[-1.5],
                                                        endList=[0.5],
                                                        overlapRatio=0.2,
                                                        levels_num=1,
                                                        input_keys=[Key("cline")],
                                                        output_keys=[Key("dp_refine1p")],
                                                        FB_keys=[Key("cline")],
                                                        layer_sizeList= [8,8,8]
                                                        )
    refine1pressureadd_net=AdditionArch(
        input1_keys=[Key("predp_refine1")],
        input2_keys=[Key("dp_refine1p")],
        output_keys=[Key("dp_refine1")],
        )
    vel_module=AdditionArch(
        input1_keys=[Key("u0"),Key("v0"),Key("w0"),Key("p0")],
        input2_keys=[Key("du_pr"),Key("dv_pr"),Key("dw_pr"),Key("dp_pr")],
        output_keys=[ Key("u"),Key("v"),Key("w"),Key("p")],
        )
    velrefine_module=AdditionArch(
        input1_keys=[Key("predu"),Key("predv"),Key("predw"),Key("dp")],
        input2_keys=[Key("predu_refine1"),Key("predv_refine1"),Key("predw_refine1"),Key("dp_refine1")],
        output_keys=[ Key("predu_pr"),Key("predv_pr"),Key("predw_pr"),Key("dp_pr")],
        )
    ...
    sethb_net = MultiplicationArch(
        input1_keys=[Key("predu_pr"),Key("predv_pr"),Key("predw_pr")],
        input2_keys=[Key("hb")],
        output_keys=[Key("du_pr"),Key("dv_pr"),Key("dw_pr")],
    )
    ...
    nodes = (
        ...
        + [refine1_net.make_node(name="refine1_net")]
        + [refine1presure_net.make_node(name="refine1presure_net")]
        + [refine1pressureadd_net.make_node(name="refine1pressureadd_net")]
        + [initVel_module.make_node(name="initVel_net")]
        + [vel_module.make_node(name="vel_net")]
        + [velrefine_module.make_node(name="velrefine_net")]
    ...
    #!!Load backup
    result_net.load_state_dict(
        torch.load(
            "/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/result_net.0.pth"
        ))
    for param in result_net.parameters():
        param.requires_grad = False
    ...
    interior = PointwiseInteriorConstraint(
        ...
        bounds=Bounds({x: (channel_length_nd[0],channel_length_nd[1]*0.8), y: channel_width_nd, z: channel_width_nd}),
        parameterization=param_ranges,
        batch_per_epoch= 1000*batchsizefactor,
    )
