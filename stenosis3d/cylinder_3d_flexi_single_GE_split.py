...
def run(cfg: ModulusConfig) -> None:
    ...
    radii_net = FullyConnectedFlexiLayerSizeArch(
            input_keys=[Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z")],
    ...
    cline_net = FullyConnectedFlexiLayerSizeArch(
            ...
            output_keys=[Key("duweights",size=total_modes),Key("dvweights",size=total_modes),Key("dwweights",size=total_modes),Key("dpweights",size=total_modes)],
            layer_sizeList=[1024,1024,1024],
        )
    get_du_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("duweights",size=total_modes)],
        output_keys=[Key("du")],
    )
    get_dv_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("dvweights",size=total_modes)],
        output_keys=[ Key("dv")],
    )
    get_dw_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("dwweights",size=total_modes)],
        output_keys=[ Key("dw")],
    )
    get_p_net = SumMultiplicationArch(
        input1_keys=[Key("modes",size=total_modes)],
        input2_keys=[Key("dpweights",size=total_modes)],
        output_keys=[ Key("dp")],
    )
