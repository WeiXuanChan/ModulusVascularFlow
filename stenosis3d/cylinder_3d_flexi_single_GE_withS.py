...
class velInit(torch.nn.Module):
    def __init__(self,u_max,init_p,p0=True):
        super().__init__()
        self.p0=p0
    ...
    def forward(self,x):#dissq,arearatio,vecx,vecy,vecz,Vin,cline
        ...
        if self.p0:
            ...
            return vel*x[...,2:5]
class getVecNormal(torch.nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self,x):#vecx,vecy,vecz
        xvec=x/torch.sum(x**2.,dim=-1,keepdim=True)
        norm=1./torch.sqrt(xvec[...,0:1]**2.+xvec[...,2:3]**2.)
        yvec=norm*torch.cat((xvec[...,0:1]*xvec[...,1:2],-xvec[...,1:2]*xvec[...,2:3],-xvec[...,0:1]**2.-xvec[...,2:3]**2.),dim=-1)
        zvec=torch.cat((-xvec[...,2:3],xvec[...,1:2]*0.,xvec[...,0:1]),dim=-1)
        return torch.cat((xvec,yvec,zvec),dim=-1)
import itertools
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3d/outputs/"+sys.argv[0][:-9]+"/flow_network.0.pth","/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    # physical quantities
    ...
    vecnorm_module=CustomModuleArch(
        [Key("vec_x"),Key("vec_y"),Key("vec_z")],
        [Key("xvec_x"),Key("xvec_y"),Key("xvec_z"),Key("yvec_x"),Key("yvec_y"),Key("yvec_z"),Key("zvec_x"),Key("zvec_y"),Key("zvec_z")],
        module=getVecNormal()
        )
    ...
    total_modes=1024
    radii_net = FullyConnectedFlexiLayerSizeArch(
            input_keys=[Key("radius_y"),Key("radius_z")],
            output_keys=[Key("normmodes",size=int(total_modes/2.)),Key("othermodes",size=int(total_modes/2.))],
            layer_sizeList=[1024,1024,1024],
        )
    cline_net = FullyConnectedFlexiLayerSizeArch(
            input_keys=[Key("cline"),Key("xvec_x"),Key("xvec_y"),Key("xvec_z"),Key("yvec_x"),Key("yvec_y"),Key("yvec_z"),Key("zvec_x"),Key("zvec_y"),Key("zvec_z"),Key("arearatio")],
            output_keys=[Key("normweights",size=int(total_modes/2.)),Key("other1weights",size=total_modes),Key("other2weights",size=total_modes),Key("pweights",size=total_modes),Key("pbias")],
            layer_sizeList=[1024,512],
        )
    get_norm_net = SumMultiplicationArch(
        input1_keys=[Key("normmodes",size=int(total_modes/2.))],
        input2_keys=[Key("normweights",size=int(total_modes/2.))],
        output_keys=[Key("normW")],
    )
    get_other1_net = SumMultiplicationArch(
        input1_keys=[Key("normmodes",size=int(total_modes/2.)),Key("othermodes",size=int(total_modes/2.))],
        input2_keys=[Key("other1weights",size=total_modes)],
        output_keys=[Key("other1W")],
    )
    get_other2_net = SumMultiplicationArch(
        input1_keys=[Key("normmodes",size=int(total_modes/2.)),Key("othermodes",size=int(total_modes/2.))],
        input2_keys=[Key("other2weights",size=total_modes)],
        output_keys=[Key("other2W")],
    )
    get_du_net = SumMultiplicationArch(
        input1_keys=[Key("normW"),Key("other1W"),Key("other2W")],
        input2_keys=[Key("xvec_x"),Key("yvec_x"),Key("zvec_x")],
        output_keys=[ Key("du")],
    )
    get_dv_net = SumMultiplicationArch(
        input1_keys=[Key("normW"),Key("other1W"),Key("other2W")],
        input2_keys=[Key("xvec_y"),Key("yvec_y"),Key("zvec_y")],
        output_keys=[ Key("dv")],
    )
    get_dw_net = SumMultiplicationArch(
        input1_keys=[Key("normW"),Key("other1W"),Key("other2W")],
        input2_keys=[Key("xvec_z"),Key("yvec_z"),Key("zvec_z")],
        output_keys=[ Key("dw")],
    )
    get_p_net = SumMultiplicationArch(
        input1_keys=[Key("normmodes",size=int(total_modes/2.)),Key("othermodes",size=int(total_modes/2.))],
        input2_keys=[Key("pweights",size=total_modes)],
        output_keys=[ Key("p_w")],
    )
    pbias_module=AdditionArch(
        input1_keys=[Key("p_w")],
        input2_keys=[Key("pbias")],
        output_keys=[Key("p")],
        )
    initVel_module=CustomModuleArch(
        [Key("dissq"),Key("arearatio"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("Vin"),Key("cline")],
        [Key("u0"),Key("v0"),Key("w0")],
        module=velInit(nd.ndim(inlet_u),nd.ndim(init_p),p0=False)
        )
    vel_module=AdditionArch(
        input1_keys=[Key("u0"),Key("v0"),Key("w0")],
        input2_keys=[Key("du"),Key("dv"),Key("dw")],
    ...
    Qsummode_net=SumPoolArch(
        input_keys=[Key("normmodes",size=int(total_modes/2.))],
        output_keys=[Key("meannormmodes",size=int(total_modes/2.))],
        pooldim=0,
        keepdim=True,
        keepview=True,
    )
    nodes = (
        ...
        + [vecnorm_module.make_node(name="vecnorm_net")]
        + [radii_net.make_node(name="radii_net")]
        + [cline_net.make_node(name="cline_net")]
        + [get_norm_net.make_node(name="get_norm_net")]
        + [get_other1_net.make_node(name="get_other1_net")]
        + [get_other2_net.make_node(name="get_other2_net")]
        + [pbias_module.make_node(name="pbias_net")]
        + [get_du_net.make_node(name="get_du_net")]
        + [get_dv_net.make_node(name="get_dv_net")]
        + [get_dw_net.make_node(name="get_dw_net")]
        + [get_p_net.make_node(name="get_p_net")]
        + [Qsummode_net.make_node(name="Qsummode_net")]
    ...
    outlet = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=tubeoutlet,
        outvar={"p": nd.ndim(outlet_p),"meannormmodes":0.},
        batch_size=cfg.batch_size.inlet,
        parameterization=param_ranges,
        batch_per_epoch= 1000*batchsizefactor,
    )
