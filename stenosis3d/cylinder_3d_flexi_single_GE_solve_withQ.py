...
def run(cfg: ModulusConfig) -> None:
    ...
    flow_net = FullyConnectedArch(
        ...
        output_keys=[Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq")],
        layer_size=1024,
        nr_layers=4
    )
    omega_net = FullyConnectedFlexiLayerSizeArch(
            input_keys=[Key("cline"),Key("radius_y"),Key("radius_z"),Key("vec_x"),Key("vec_y"),Key("vec_z"),Key("dissq")],
            output_keys=[Key("preomega",size=2048)],
            layer_sizeList=[],
        )
    omegamultiplier_net = CustomModuleArch(
        [Key("preomega",size=2048)],
        [Key("omega",size=2048)],
        module=omegamultiplier(7,2048)
        )
    CosSin_net=CosSinArch(
        [Key("omega",size=2048)],
        [Key("u"),Key("v"),Key("w"),Key("p")],
        sin_layer=False
    )
    ...
    nodes = (
        ...
        + [omega_net.make_node(name="omega_net")]
        + [omegamultiplier_net.make_node(name="omegamultiplier_net")]
        + [CosSin_net.make_node(name="CosSin_net")]
