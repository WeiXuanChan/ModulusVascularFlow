...
class SymmetricMomentumLoss(PDE):
    def __init__(self, factor):
        # coordinates
        mo_x=Symbol("momentum_x")
        mo_y=Symbol("momentum_y")
        ...
        # set equations
        self.equations = {}
        self.equations["sym_momentum_x"] = (
            mo_x*(1+factor*vec_x)
        )  # "custom_pde" key name will be used in constraints
        self.equations["sym_momentum_y"] = (
            mo_y*(1+factor*vec_y)
        )
        self.equations["sym_momentum_z"] = (
            mo_z*(1+factor*vec_z)
        )
import torch
...
def run(cfg: ModulusConfig) -> None:
    ...
    if not(os.path.isfile("/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")):
        ...
        shutil.copy("/examples/stenosis3d/outputs/"+sys.argv[0][:-19]+"/flow_network.0.pth","/examples/stenosis3d/outputs/"+sys.argv[0][:-3]+"/flow_network.0.pth")
    # physical quantities
    ...
    asy_ns=SymmetricMomentumLoss(0.1)
    ...
    interior = PointwiseInteriorConstraint(
        nodes=nodes,
        geometry=volume_geo,
        outvar={"continuity": 0, "sym_momentum_x": 0, "sym_momentum_y": 0, "sym_momentum_z": 0},
    ...
    # make solver
    slv = Solver_ReduceLROnPlateauLoss(cfg, domain,batch_per_epoch=1000*batchsizefactor*batchsize_correction,ReduceLROnPlateau_DictConFig={"min_lr":cfg.optimizer.lr*0.01})
