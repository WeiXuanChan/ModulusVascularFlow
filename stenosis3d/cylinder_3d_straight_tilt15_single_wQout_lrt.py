...
class Newcoord(torch.nn.Module):
    ...
    def forward(self,x):
        return self.s*x[...,0:3]
class rotate_y(torch.nn.Module):
    def __init__(self,deg):
        super().__init__()
        rad=torch.tensor(deg/180.*np.pi)
        cosrad=np.cos(rad)
        sinrad=np.sin(rad)
        rotate=torch.tensor([[cosrad,sinrad,0.],[-sinrad,cosrad,0.],[0.,0.,1.]],dtype=torch.float).T
        self.register_buffer("rotate", rotate, persistent=False)
    def forward(self,x):
        y=torch.transpose(x.reshape(x.size()[:-1]+(int(x.size()[-1]/3),3)),-1,-2)
        y=torch.transpose(torch.matmul(self.rotate,y),-1,-2)
        return y.reshape(x.size())
import itertools
straighten=0.
rotate_deg=15.
...
def run(cfg: ModulusConfig) -> None:
    ...
    Stenosis_coordTransform=CustomModuleArch(
        ...
        [Key("x_case0"), Key("y_case0"),Key("z_case0"),Key("delta_yx"),Key("delta_zx")],
        module=Stenosis_module
        )
    vec_ref_mod=CustomModuleArch(
        [Key("delta_yx"),Key("delta_zx")],
        [Key("ref_vec_x0"), Key("ref_vec_y0"), Key("ref_vec_z0")],
        module=vec_ref()
        )
    ...
    rotate_y_net=CustomModuleArch(
        [Key("x_case0"), Key("y_case0"),Key("z_case0"),Key("ref_vec_x0"), Key("ref_vec_y0"), Key("ref_vec_z0")],
        [Key("x_case"), Key("y_case"),Key("z_case"),Key("ref_vec_x"), Key("ref_vec_y"), Key("ref_vec_z")],
        module=rotate_y(rotate_deg)
        )
    ...
    nodes = (
        ...
        +[rotate_y_net.make_node(name='rotate_y_net')]
        + ns.make_nodes()
    ...
    openfoam_var = csv_to_dict(
        to_absolute_path("/examples/stenosis3d/straighten_0.csv"), mapping
    )
    openfoam_invar_numpy_csv = {"z_case":openfoam_var["z_case"]/0.012}
    openfoam_invar_numpy_csv["x_case"]=(openfoam_var["x_case"]*np.cos(rotate_deg/180.*np.pi)-openfoam_var["y_case"]*np.sin(rotate_deg/180.*np.pi))/0.012
    openfoam_invar_numpy_csv["y_case"]=(openfoam_var["y_case"]*np.cos(rotate_deg/180.*np.pi)+openfoam_var["x_case"]*np.sin(rotate_deg/180.*np.pi))/0.012

    openfoam_inferencer_csv=PointwiseInferencer(
    nodes=nodes, invar=openfoam_invar_numpy_csv, output_names=["u_scaled", "v_scaled", "w_scaled", "p_scaled"]
    )
    domain.add_inferencer(openfoam_inferencer_csv, "inf_data_csv")
    # make solver
